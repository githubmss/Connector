...
class PlaceOrder extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($this->customerSession->isLoggedIn()) {
            ...
            if ($totalItems > 0) {
                ...
                isset($registration_id)?$finalJosn['registration_id']:"";
    ...
    private function _initToken($setToken = null)
    {
        ...
        $objectData = \Magento\Framework\App\ObjectManager::getInstance();
        $getSession = $objectData->get('\Magento\Framework\Session\Generic');
        $params     = $this->getParams();
        $finalJosn  = $this->jsonHelper->jsonDecode($params, true);
        $setToken   = $finalJosn['token'];
        $getSession->setExpressCheckoutToken($setToken);
        if ($setToken) {
            if ($setToken !== $getSession->getExpressCheckoutToken()) {
                throw new \Magento\Framework\Exception\LocalizedException(
                    __('A wrong PayPal Express Checkout Token is specified.')
                );
            }
        } else {
            $setToken = $getSession->getExpressCheckoutToken();
        }
        return $setToken;
    }
